...
float BKE_nurb_calc_length(const Nurb *nu, int resolution)
{
  ...
  if (nu->type == CU_POLY) {
    ...
    if (nu->pntsv == 1) {
      ...
      BKE_nurb_makeCurve(nu, points, NULL, NULL, NULL, NULL, resolu, sizeof(float[3]));
                        ...
                        int stride)
{
  ...
  while (resolu--) {
    ...
    if (radius_normal_fp) {
      radius_normal_fp = POINTER_OFFSET(radius_normal_fp, stride);
    }
    if (weight_fp) {
                         ...
                         int stride)
{
  ...
  for (a = 0; a < resolu; a++, fac += dfac) {
    ...
    if (radius_normal_array) {
      ...
      radius_normal_array = POINTER_OFFSET(radius_normal_array, stride);
    }
      ...
      while (a--) {
        if (prevbezt->h2 == HD_VECT && bezt->h1 == HD_VECT) {
          ...
          tilt_bezpart(prevbezt,
                       ...
                       do_radius ? &bevp->radius_normal : NULL,
